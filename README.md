...
| GatherBuddy Reborn | `7.3.8.141` | 基於上游 `7.3.8.14`，即最後一個仍為 Dalamud API 13 的版本（`7.4.0.0` 起改用 API 14、`7.5.x` 為 API 15，台服都無法載入）。已處理台服 `ClientLanguage`，並把台服尚未開放的魚類／魚餌／藏寶圖／採集點改為靜默略過；介面繁體中文化 |
| Rotation Solver Reborn | `7.3.8.271` | 基於上游 `7.3.8.27` 對台服 API 13 重新編譯；DTR 介面已對齊台服，保留原生 AutoDuty IPC 與同版內建技能組 |
...
- Rotation Solver Reborn 上游預編譯的 `7.3.8.27` 使用不同的 DTR `OnClick` 二進位簽名；本站 `7.3.8.271` 已針對台服 runtime 重新綁定。

這些 `7.3.8.x` 套件的遊戲資料基底更接近 `02` 版 Dalamud，但上游預編譯 DLL 仍可能與台服 runtime 不相容。目前 GatherBuddy 與 Rotation Solver 均已改用 `7.3.8.x` 原始碼，並針對台服 API 13 重新編譯或加入必要的資料相容修正。
...
| GatherBuddy Reborn 繁體中文台服版 | `7.3.8.141` | 13 | 介面繁體中文化；已升到上游最後一個 API 13 基底 `7.3.8.14`，含台服資料相容修正 |
| Rotation Solver Reborn | `7.3.8.271` | 13 | 7.38 技能組基底；台服 DTR 相容重編譯，原生支援 AutoDuty IPC |
